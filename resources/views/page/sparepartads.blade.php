...
<div class="search-data">
	...
	@foreach($csp as $sp)
      @foreach($favorites as $favorite)
		...
		<?php
	$image =explode('|',$sp->images);
	?>
	
	@if($logged == 'ok')
		@if($favorite->username == $Logged['logusername'] && $favorite->type == $sp->type && $favorite->ad_username == $sp->user_name && $favorite->model == $sp->car_model && $favorite->price == $sp->price && $favorite->brand == $sp->brand && $favorite->created_at == $sp->created_at && $favorite->image == $image[0])
	...
	<form class="myForm" action="" method="POST" enctype="multipart/form-data">
		...
		<div class="container">
			...
			<input type="hidden" name="ad_username" value="{{$sp->user_name}}">
			<input type="hidden" name="image" value="{{$image[0]}}">
			<input type="hidden" name="model" value="{{$sp->car_model}}">
			...
			<input type="hidden" name="time" value="{{($sp->created_at)}}">
				...
				<span class="tit2 "style="color:black; margin-left:100px; position: absolute;
				  ...
				  type:
						...
						<span class="tit2 "style="color:black; position:absolute;
                            ...
                            left: 235px;
                            width: 500px;
                            height: 30px" >{{$sp->type}}</span>
		
						<br>
						<br>
					  <div style="color:black; margin-top:48px; margin-left:-35px">
					  {{Carbon\Carbon::parse($sp->created_at)->diffForHumans()}}
							</div>
										
							<a href="{{ url('/detail ads')}}/<?php echo $sp->user_name;?>/<?php echo $sp->created_at;?>';" class="btn btn-primary" style="width: 200px;margin-left: 983px;margin-top: 11px;"  > show</a>
	...
	@elseif($favorite->username !== $Logged['logusername'] && $favorite->type !== $sp->type && $favorite->ad_username !== $sp->user_name && $favorite->model !== $sp->car_model && $favorite->price !== $sp->price && $favorite->brand !== $sp->brand && $favorite->created_at !== $sp->created_at && $favorite->image !== $image[0])
