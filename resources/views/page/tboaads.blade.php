...
<div class="search-data">
	...
	@foreach($favorites as $favorite)
		<?php
	$image =explode('|',$t->images);
	?>
	@if($logged == 'ok')
		@if($favorite->username == $Logged['logusername'] && $favorite->type == $t->type && $favorite->ad_username == $t->user_name && $favorite->model == $t->car_model && $favorite->price == $t->price && $favorite->brand == $t->brand && $favorite->created_at == $t->created_at && $favorite->image == $image[0])

			<form class="myForm" action="" method="POST" enctype="multipart/form-data">
		@csrf
			<div class="container">
				<input type="hidden" name="username" value="{{ $Logged['logusername'] }}">
				<input type="hidden" name="ad_username" value="{{$t->user_name}}">
				<input type="hidden" name="image" value="{{$image[0]}}">
				<input type="hidden" name="model" value="{{$t->car_model}}">
				<input type="hidden" name="price" value="{{$t->price}}">
				<input type="hidden" name="brand" value="{{$t->brand}}">
				<input type="hidden" name="type" value="{{$t->type}}">
				<input type="hidden" name="time" value="{{($t->created_at)}}">
				...
				<span class="tit2 " style="color:black; margin-left:100px; position: absolute;
				  ...
				  type:
					  ...
					  {{Carbon\Carbon::parse($t->created_at)->diffForHumans()}}
							...
							<a href="{{ url('/detail ads')}}/<?php echo $t->user_name;?>/<?php echo $t->created_at;?>';" class="btn btn-primary" style="width: 200px;margin-left: 983px;margin-top: 11px;"  > show</a>
	...
	@elseif($favorite->username !== $Logged['logusername'] && $favorite->type !== $t->type && $favorite->ad_username !== $t->user_name && $favorite->model !== $t->car_model && $favorite->price !== $t->price && $favorite->brand !== $t->brand && $favorite->created_at !== $t->created_at && $favorite->image !== $image[0])
	<form class="myForm" action="" method="POST" enctype="multipart/form-data">
		@csrf
			<div class="container">
				<input type="hidden" name="username" value="{{$Logged['logusername']}}">
