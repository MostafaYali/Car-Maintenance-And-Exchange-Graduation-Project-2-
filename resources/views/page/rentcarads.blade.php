...
@php($i=1)
        ...
        <?php
	$image =explode('|',$r->images);
	?>
	@if($logged == 'ok')
			  @if($favorite->username == $Logged['logusername'] && $favorite->ad_username == $r->user_name && $favorite->model == $r->car_model && $favorite->price == $r->price && $favorite->brand == $r->brand && $favorite->created_at == $r->created_at && $favorite->image == $image[0])

<form class="myForm" action="" method="POST" enctype="multipart/form-data">
		@csrf
<div class="container">
		<input type="hidden" name="username" value="{{ $Logged['logusername'] }}">
		<input type="hidden" name="ad_username" value="{{$r->user_name}}">
		<input type="hidden" name="image" value="{{$image[0]}}">
		<input type="hidden" name="model" value="{{$r->car_model}}">
		<input type="hidden" name="price" value="{{$r->price}}">
		<input type="hidden" name="brand" value="{{$r->brand}}">
		<input type="hidden" name="time" value="{{($r->created_at)}}">
				...
				<br>
					...
					<div style="color:black; margin-top:34px; margin-left:-35px" >
					  {{Carbon\Carbon::parse($r->created_at)->diffForHumans()}}
							...
							<a href="{{ url('/detail ads')}}/<?php echo $r->user_name;?>/<?php echo $r->created_at;?>';" class="btn btn-primary" style="width: 200px;margin-left: 983px;"  > show</a>
...
@elseif($favorite->username !== $Logged['logusername'] && $favorite->ad_username !== $r->user_name && $favorite->model !== $r->car_model && $favorite->price !== $r->price && $favorite->brand !== $r->brand && $favorite->created_at !== $r->created_at && $favorite->image !== $image[0])
<form class="myForm" action="" method="POST" enctype="multipart/form-data">
		@csrf
<div class="container">
		<input type="hidden" name="username" value="{{$Logged['logusername']}}">
